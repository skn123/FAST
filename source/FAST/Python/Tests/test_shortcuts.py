...
def test_display2D():
    ...
    segmentation = fast.BinaryThresholding(100).connect(importer)
...
def test_display2D_return_window():
    ...
    renderer = fast.SegmentationRenderer().connect(segmentation)
...
def test_display3D_slicer():
    importer = fast.ImageFileImporter(fast.Config.getTestDataPath() + 'CT/CT-Thorax.mhd')
    fast.display3D(importer, timeout=timeout)


def test_display3D_slicer_segmentation():
    importer = fast.ImageFileImporter(fast.Config.getTestDataPath() + 'CT/CT-Thorax.mhd')
    segmentation = fast.BinaryThresholding(300).connect(importer)
    fast.display3D(
        importer,
        segmentation,
        segmentationColors={1: fast.Color.Red()},
