...
/*
TEST_CASE("Correct output with small 3x3 2D image as input to GaussianSmoothing on OpenCLDevice", "[fast][GaussianSmoothing]") {
...
// TODO fix this test
TEST_CASE("Correct output with small 3x3 2D image as input to GaussianSmoothing on Host", "[fast][GaussianSmoothing]") {
    GaussianSmoothing::pointer filter = GaussianSmoothing::New();
    filter->setMaskSize(3);
    filter->setStandardDeviation(1.0);
...
TEST_CASE("Correct output with small 3x3x3 3D image as input to GaussianSmoothing on OpenCLDevice", "[fast][GaussianSmoothing]") {
    DeviceManager* deviceManager = DeviceManager::getInstance();
    OpenCLDevice::pointer device = deviceManager->getOneOpenCLDevice();
    GaussianSmoothing::pointer filter = GaussianSmoothing::New();
    filter->setMaskSize(3);
...
TEST_CASE("Correct output with small 3x3x3 3D image as input to GaussianSmoothing on Host", "[fast][GaussianSmoothing]") {
    GaussianSmoothing::pointer filter = GaussianSmoothing::New();
    filter->setMaskSize(3);
    filter->setStandardDeviation(1.0);
