...
__kernel void gaussianSmoothing(
    ...
    for(int y = -halfSize.y; y <= halfSize.y; ++y) {
        ...
        if(dataType == CLK_FLOAT) {
            sum += mask[x+halfSize.x+(y+halfSize.x)*maskSize.x]*read_imagef(input, sampler, pos+offset);
        } else if(dataType == CLK_UNSIGNED_INT8 || dataType == CLK_UNSIGNED_INT16 || dataType == CLK_UNSIGNED_INT32) {
            sum += mask[x+halfSize.x+(y+halfSize.y)*maskSize.x]*convert_float4(read_imageui(input, sampler, pos+offset));
        } else {
            sum += mask[x+halfSize.x+(y+halfSize.y)*maskSize.x]*convert_float4(read_imagei(input, sampler, pos+offset));
        }
    }}
