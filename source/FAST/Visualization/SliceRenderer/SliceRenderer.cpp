...
void SliceRenderer::execute() {
	...
	for (unsigned int inputIndex = 1; inputIndex < nrOfInputData; inputIndex++)
	{
		...
		{
			transform = SceneGraph::getAffineTransformationFromData(mImagesToRender[inputIndex]);
			transformationMatix = transform.inverse().data();
		}
		else
		{
			transformationMatix = transform.data();
		}
...
void SliceRenderer::draw() {
	...
	for(uint inputNr = 0; inputNr < getNrOfInputData(); inputNr++) {
        AffineTransformation transform = SceneGraph::getAffineTransformationFromData(mImagesToRender[inputNr]);
        transform.scale(mImagesToRender[inputNr]->getSpacing());
...
BoundingBox SliceRenderer::getBoundingBox() {
    ...
    AffineTransformation transform = SceneGraph::getAffineTransformationFromData(mImagesToRender[0]); // TODO fix
